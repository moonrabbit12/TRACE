...
def main():
    ...
    if 'vicuna' in args.model_name_or_path:
        ...
        prev_model = create_hf_model(AutoModelForCausalLM,
                                args.model_name_or_path,
                                tokenizer,
                                ds_config=ds_config,
                                disable_dropout=args.disable_dropout,
    ...
    for dataset in Datasets:
        train_dataloader, eval_dataloader, test_dataloader = get_dataset(dataset)
    ...
    prev_optimizer, prev_lr_scheduler = get_optimizer(prev_model)
    prev_model, prev_optimizer, _, prev_lr_scheduler = deepspeed.initialize(
        model=prev_model,
        optimizer=prev_optimizer,
        args=args,
        config=ds_config,
        lr_scheduler=prev_lr_scheduler,
        dist_init_required=True)
    
    # TODO:
    # set prev model to eval()
    # or no grad
    ...
    def train_one_task(task, i_task, epochs, projection_configs):
        ...
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                prev_outputs = prev_model(**batch, use_cache=False)
                student_logits = outputs.logits
                teacher_logits = prev_outputs.logits.detach()
                ce_loss = outputs.loss
                kd_loss = F.kl_div(F.log_softmax(student_logits, dim=-1), F.log_softmax(teacher_logits, dim=-1), reduction='batchmean', log_target=True)
                loss = 0.999*ce_loss + 0.001*kd_loss

                # Update the description to include current step and loss, if needed
                if args.global_rank == 0:
                    # Update the progress bar
                    progress_bar.update(1)
                    description = f"Epoch {epoch+1}, Step {step}, Loss: {loss.item():.4f}, CE Loss: {ce_loss.item():.4f}, KL Div: {kd_loss.item():.4f}"
