...
def save_plot_to_pdf_improved(data, output_directory, name, cfg, baselines):
    ...
    filename = f"{cfg_name}_{name}_CLB1.pdf"
...
def save_plot_to_pdf(data, output_directory, name, cfg):
    ...
    namefile = cfg_name + name + 'CLB1.pdf'
...
expected_values = {
    'cl_method': ['svd_replay', 'PCA', 'svd_replay_alignment'],
...
filename_pattern = re.compile(r'^results-([0-3])-([0-3])-(.+)\.json$')
...
for key, metric_values in search_table.items():
    ...
    if config_key not in tables:
        tables[config_key] = pd.DataFrame(index=range(4), columns=range(4))
...
outputname = CL_method + '_ci_tables_CLB1.pdf'
...
output_pdf_path = os.path.join(output_directory, 'ci_baseline_tables_CLB1.pdf')
