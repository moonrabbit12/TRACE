...
class CustomLlamaMLP(LlamaMLP):
    ...
    def forward(self, x, projection_config):
        if self.pretraining_tp > 1:
            slice = self.intermediate_size // self.pretraining_tp
            gate_proj_slices = self.gate_proj.weight.split(slice, dim=0)
            up_proj_slices = self.up_proj.weight.split(slice, dim=0)
...
class CustomLlamaDecoderLayer(LlamaDecoderLayer):
    ...
    def forward(
        ...
        output_attentions: Optional[bool] = False,
        use_cache: Optional[bool] = False,
        projection_config: Optional[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]] = None,
    ) -> Tuple[torch.FloatTensor, Optional[Tuple[torch.FloatTensor, torch.FloatTensor]]]:
        """
        Args:
            hidden_states (`torch.FloatTensor`): input to the layer of shape `(batch, seq_len, embed_dim)`
        ...
        residual = hidden_states
        hidden_states = self.post_attention_layernorm(hidden_states)
        hidden_states = self.mlp(hidden_states, projection_config)
        hidden_states = residual + hidden_states
...
class CustomLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList([CustomLlamaDecoderLayer(config) for _ in range(config.num_hidden_layers)])
        self.norm = LlamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.hidden_size = config.hidden_size
        self.gradient_checkpointing = False
        # Initialize weights and apply final processing
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        projection_configs: Optional[Tuple[List[torch.Tensor], List[torch.Tensor], List[torch.Tensor]]] = None,
    ) -> Union[Tuple, BaseModelOutputWithPast]:
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                def create_custom_forward(module):
                    def custom_forward(*inputs):
                        # None for past_key_value
                        #return module(*inputs, output_attentions=output_attentions, use_cache=None)
